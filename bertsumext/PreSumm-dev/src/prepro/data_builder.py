...
def tokenize(args):
    ...
    with open("mapping_for_corenlp.txt", "w", encoding="utf-8") as f:
        for s in stories:
...
def format_xsum_to_lines(args):
    ...
    corpus_mapping = json.load(open(pjoin(args.raw_path, 'XSum-TRAINING-DEV-TEST-SPLIT-90-5-5.json'), encoding="utf-8"))
...
def _format_xsum_to_lines(params):
    ...
    if (os.path.exists(f_src) and os.path.exists(f_tgt)):
        print(name)
        source = []
        for sent in open(f_src, encoding="utf-8"):
            source.append(sent.split())
        tgt = []
        for sent in open(f_tgt, encoding="utf-8"):
